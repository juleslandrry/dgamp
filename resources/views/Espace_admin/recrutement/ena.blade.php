...
<div class="mdg-wrap">
    ...
    <div class="top-bar">
        <h2>📋 Liste des documents ({{ count($ena) }})</h2>
    ...
    <form method="POST" action="{{ route('ena.update') }}" enctype="multipart/form-data">
        ...
        <div class="table-card">
            ...
            <div id="empty-row" class="empty-row" style="{{ count($ena) > 0 ? 'display:none;' : '' }}">
...
@foreach($ena as $doc)
    @if(!empty($doc['id']))
        <form id="delete-form-{{ $doc['id'] }}" method="POST" action="{{ route('ena.destroy', $doc['id']) }}" style="display:none;">
            @csrf
            @method('DELETE')
        </form>
    @endif
...
function updateCount() {
    const n = document.querySelectorAll('#ena-tbody tr[data-row]').length;
...
function addDocument() {
    const tbody = document.getElementById('ena-tbody');
