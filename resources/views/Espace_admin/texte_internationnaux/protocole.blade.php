...
<div class="mdg-wrap">
    ...
    <h1 class="mdg-title">Protocole DGAM</h1>
    <p class="mdg-sub">Gère la liste des protocoles affichés sur le site public.</p>
    ...
    @if(!$detection_ok)
        <div class="mdg-alert warn">⚠️ La structure du fichier n'a pas pu être détectée automatiquement. Vérifie le contenu avant d'enregistrer.</div>
    @endif

    <form method="POST" action="{{ route('protocoles.update') }}" enctype="multipart/form-data">
        @csrf

        <div id="protocoles-list">
            @foreach($protocoles as $i => $proto)
                <div class="acc-item card-block">
                    <div class="card-block-label">
                        <span class="card-num">{{ $i + 1 }}</span>
                        <span class="txt">Protocole {{ $i + 1 }}</span>
                    </div>
                    @if($i > 0)
        ...
        <button type="button" class="btn-add-card" onclick="addProtocole()">+ Ajouter un protocole</button>
...
function addProtocole() {
    ...
    wrap.innerHTML = `
        <div class="card-block-label">
            ...
            <span class="txt">Protocole ${num} (nouveau)</span>
        </div>
        ...
        <div class="mdg-field">
            ...
            <input type="text" name="intitule[]" placeholder="Ex: Intitulé du protocole">
        </div>
