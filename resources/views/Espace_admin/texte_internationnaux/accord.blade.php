...
<style>
    ...
    /* Cartes (même logique que Mot du DG : pastille d'icône + label + champ en cadre) */
...
<div class="mdg-wrap">
    ...
    <h1 class="mdg-title">Accords DGAM</h1>
    <p class="mdg-sub">Modifie un accord existant, ajoute-en un nouveau, ou remplace son fichier PDF.</p>
    ...
    @if(!$detection_ok)
        <div class="mdg-alert warn">⚠️ Les accords n'ont pas pu être détectés automatiquement. Vérifie le contenu avant d'enregistrer.</div>
    @endif

    <form method="POST" action="{{ route('accords.update') }}" enctype="multipart/form-data">
        @csrf
            ...
            @foreach($accords as $i => $accord)
                <div class="card-block">
                    <div class="card-block-label">
                        ...
                        <span class="txt">Accord {{ $i + 1 }}</span>
                    </div>
                    @if($i > 0)
                    ...
                    <div class="mdg-field">
                        ...
                        <input type="text" name="intitule[]" value="{{ $accord['intitule'] }}" placeholder="Ex: Accord N°1">
                    </div>
                    ...
                    <div class="mdg-field">
                        ...
                        <input type="hidden" name="lien[]" value="{{ $accord['lien'] }}">
                        <div class="mdg-hint">Laisse vide pour garder le fichier actuel</div>
                    </div>
                </div>
            @endforeach
        </div>
        <button type="button" class="btn-add-card" onclick="addAccord()">+ Ajouter un accord</button>
...
<script>
function addAccord() {
    const list = document.getElementById('accords-list');
    ...
    wrap.innerHTML = `
        <div class="card-block-label">
            ...
            <span class="txt">Accord ${num} (nouveau)</span>
        </div>
        ...
        <div class="mdg-field">
            ...
            <input type="text" name="intitule[]" placeholder="Ex: Accord N°${num}">
        </div>
        ...
        <div class="mdg-field">
            ...
            <div class="mdg-hint">Un fichier PDF est requis pour un nouvel accord</div>
        </div>
    `;
