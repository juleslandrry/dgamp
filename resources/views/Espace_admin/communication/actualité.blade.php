...
<style>
    ...
    .photo-preview{width:44px;height:44px;object-fit:cover;border-radius:7px;border:2px solid var(--gold);
        margin-top:8px;}
...
<div class="mdg-wrap">
    ...
    <p class="mdg-sub">Gère les articles d'actualité affichés sur le site public.</p>
    ...
    @if(!$detection_ok)
        <div class="mdg-alert warn">⚠️ Les articles n'ont pas pu être détectés automatiquement. Vérifie le contenu avant d'enregistrer.</div>
    @endif

    <form method="POST" action="{{ route('actualites.update') }}" enctype="multipart/form-data">
        @csrf
            ...
            @foreach($articles as $i => $art)
                <div class="card-block">
                    <div class="card-block-label">
                        ...
                        <span class="txt">Article {{ $i + 1 }}</span>
                    </div>
                    @if($i > 0)
                    ...
                    <div class="mdg-field">
                        <div class="mdg-label">
                            <span class="mdg-icon i-blue"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h9v10H2z"/><path d="M11 5h3v8h-2"/><path d="M4 6h5M4 8h5M4 10h3"/></svg></span>
                            Titre de l'article
                        </div>
                        <input type="text" name="titre[]" value="{{ $art['titre'] }}" placeholder="Titre de l'actualité">
                    </div>
                    ...
                    <div class="mdg-row2">
                        <div class="mdg-field">
                            ...
                            <input type="text" name="date[]" value="{{ $art['date'] }}" placeholder="Ex: Publié le : 11 Décembre 2025">
                        </div>
                        <div class="mdg-field">
                            <div class="mdg-label">
                                <span class="mdg-icon i-navy"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 8h10M8 3v10"/></svg></span>
                                Catégorie
                            </div>
                            <input type="text" name="categorie[]" value="{{ $art['categorie'] }}" placeholder="Ex: RENCONTRE, FORMATION...">
                        </div>
                    </div>

                    <div class="mdg-field">
                        <div class="mdg-label">
                            <span class="mdg-icon i-green"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h12v7H6l-3 3v-3H2z"/></svg></span>
                            Extrait / résumé
                        </div>
                        <textarea name="extrait[]">{{ $art['extrait'] }}</textarea>
                    </div>

                    <div class="mdg-field">
                        <div class="mdg-label">
                            <span class="mdg-icon i-orange"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2h5l3 3v9H4z"/><path d="M9 2v3h3"/></svg></span>
                            Lien "Lire la suite" (optionnel)
                        </div>
                        <input type="text" name="lien[]" value="{{ $art['lien'] }}" placeholder="#">
                    </div>
                    ...
                    <div class="mdg-field">
                        ...
                        <input type="hidden" name="image_actuelle[]" value="{{ $art['image'] }}">
                        @if($art['image'])
                            <img src="{{ asset($art['image']) }}" class="photo-preview">
                        @endif
                    </div>
                </div>
            @endforeach
        ...
        <button type="button" class="btn-add-card" onclick="addArticle()">+ Ajouter un article</button>
...
function addArticle() {
    ...
    wrap.innerHTML = `
        <div class="card-block-label">
            ...
            <span class="txt">Article ${num} (nouveau)</span>
        </div>
        <button type="button" class="btn-remove-card" onclick="this.parentElement.remove()">Annuler l'ajout</button>

        <div class="mdg-field">
            <div class="mdg-label"><span class="mdg-icon i-blue"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h9v10H2z"/><path d="M11 5h3v8h-2"/><path d="M4 6h5M4 8h5M4 10h3"/></svg></span>Titre de l'article</div>
            <input type="text" name="titre[]" placeholder="Titre de l'actualité">
        </div>

        <div class="mdg-row2">
            <div class="mdg-field">
                <div class="mdg-label"><span class="mdg-icon i-orange"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="12" height="11" rx="1.2"/><path d="M2 6.5h12M5 2v3M11 2v3"/></svg></span>Date de publication</div>
                <input type="text" name="date[]" placeholder="Ex: Publié le : 21 Juillet 2026">
            </div>
            <div class="mdg-field">
                <div class="mdg-label"><span class="mdg-icon i-navy"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 8h10M8 3v10"/></svg></span>Catégorie</div>
                <input type="text" name="categorie[]" placeholder="Ex: RENCONTRE, FORMATION...">
            </div>
        </div>

        <div class="mdg-field">
            <div class="mdg-label"><span class="mdg-icon i-green"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h12v7H6l-3 3v-3H2z"/></svg></span>Extrait / résumé</div>
            <textarea name="extrait[]"></textarea>
        </div>

        <div class="mdg-field">
            <div class="mdg-label"><span class="mdg-icon i-orange"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4 2h5l3 3v9H4z"/><path d="M9 2v3h3"/></svg></span>Lien "Lire la suite" (optionnel)</div>
            <input type="text" name="lien[]" placeholder="#">
        </div>

        <div class="mdg-field">
            <div class="mdg-label"><span class="mdg-icon i-gold"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="12" height="10" rx="1.2"/><circle cx="5.5" cy="6.5" r="1.1"/><path d="M2.5 11.5l3.5-3.5L9 11l2-2 2.5 2.5"/></svg></span>Photo de l'article</div>
            <input type="file" name="image[]" accept="image/*">
