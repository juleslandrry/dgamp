...
<style>
    ...
    .mdg-current-empty{font-size:11.5px;color:#8A6D14;margin-top:6px;}
...
<div class="mdg-wrap">
    ...
    @if(!$detection_ok)
        <div class="mdg-alert warn">⚠️ Les communiqués n'ont pas pu être détectés automatiquement. Vérifie le contenu avant d'enregistrer.</div>
    @endif

    <form method="POST" action="{{ route('communiques.update') }}" enctype="multipart/form-data">
        @csrf

        <div id="communiques-list">
            @foreach($communiques as $i => $com)
                <div class="card-block">
                    <div class="card-block-label">
                        <span class="card-num">{{ $i + 1 }}</span>
                        <span class="txt">Communiqué {{ $i + 1 }}</span>
                    </div>
                    @if($i > 0)
                    ...
                    <div class="mdg-row2">
                        <div class="mdg-field">
                            ...
                            <input type="text" name="titre[]" value="{{ $com['titre'] }}" placeholder="Ex: Arrêté n°332 du 26 février 2020...">
                        </div>
                        <div class="mdg-field">
                            <div class="mdg-label">
                            ...
                            <input type="text" name="format[]" value="{{ $com['format'] }}" placeholder="PDF">
                        </div>
                    </div>
                    ...
                    <div class="mdg-field">
                        ...
                        <input type="text" name="description[]" value="{{ $com['description'] }}" placeholder="Description courte du document">
                    </div>

                    <div class="mdg-field">
                        <div class="mdg-label">
                            <span class="mdg-icon i-orange"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M8 1v9M4.5 6.5L8 10l3.5-3.5M2 12v2h12v-2"/></svg></span>
                            Fichier PDF
                        </div>
                        <input type="file" name="fichier[]" accept="application/pdf">
                        <input type="hidden" name="lien[]" value="{{ $com['lien'] }}">
                        @if($com['lien'] && $com['lien'] !== '#')
                            <div class="mdg-current">📄 <a href="{{ asset($com['lien']) }}" target="_blank">Fichier actuel</a></div>
                        @else
                            <div class="mdg-current-empty">⚠️ Aucun fichier renseigné pour le moment</div>
                        @endif
                    </div>
                </div>
            @endforeach
        ...
        <button type="button" class="btn-add-card" onclick="addCommunique()">+ Ajouter un communiqué</button>
...
function addCommunique() {
    ...
    wrap.innerHTML = `
        <div class="card-block-label">
            ...
            <span class="txt">Communiqué ${num} (nouveau)</span>
        </div>
        ...
        <div class="mdg-row2">
            <div class="mdg-field">
                ...
                <input type="text" name="titre[]" placeholder="Titre du communiqué">
            </div>
            <div class="mdg-field">
                <div class="mdg-label">
                    <span class="mdg-icon i-gold"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="12" height="12" rx="2"/><path d="M5 6h6M5 9h4"/></svg></span>
        ...
        <div class="mdg-field">
            ...
            <input type="text" name="description[]" placeholder="Description courte du document">
        </div>
