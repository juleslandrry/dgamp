...
<section class="paramil-hero">
    ...
    <div class="paramil-hero-content">
        <span class="staff-badge">{{ $data->badge ?? 'Administration Centrale' }}</span>
        <h2 class="staff-title">{{ $data->titre ?? 'Personnel Interministériel' }}</h2>
        <p class="staff-description">
            {{ $data->hero_description ?? "L'excellence administrative au service de l'État. Le personnel interministériel assure la coordination et le suivi des dossiers transversaux de la DGAM." }}
        </p>
    </div>
</section>
...
<section class="paramil-split">
    <div class="paramil-split-container">
        <div class="paramil-split-text">
            ...
            <h3 class="paramil-split-title">{{ $data->section_titre ?? 'Le Personnel Interministériel' }}</h3>

            <div class="paramil-split-body">
                {!! $data->section_texte ?? "<p>Le contenu détaillé du personnel interministériel sera bientôt disponible.</p>" !!}
            </div>
        ...
        <div class="paramil-split-image">
            <img src="{{ $data && $data->section_image ? asset('storage/'.$data->section_image) : asset('assets/images/image33.jpeg') }}" alt="{{ $data->section_titre ?? 'Personnel Interministériel' }}">
        </div>
    </div>
</section>
